#!/usr/bin/env python
""" Software to create Precision-Recall-Gain curves and calibrated metrics, based on the work of:
...
Code was taken from:
https://github.com/wissam-sib/calibrated_metrics

based on their original paper "Master your Metrics with Calibration" Wissam Siblini, Jordan Fréry, Liyun He-Guelton, Frédéric Oblé and Yi-Qing Wang (IDA 2020)
...
def bestf1score(y_true, y_pred, pi0=None):
    ...
    precision, recall, thresholds = precision_recall_curve(y_true, y_pred, pi0=pi0)
    fscores = (2*precision*recall)/(precision+recall)
    fscores = np.nan_to_num(fscores,nan=0, posinf=0, neginf=0)
    
    return np.max(fscores), thresholds[np.argmax(fscores)]
...
def recall_gain(tp, fn, fp, tn,pi0):
...
def _create_crossing_points(points, n_pos, n_neg,pi0):
    ...
    for i in indices:
        ...
        new_rec_gain = recall_gain(new_point[key_indices_1['TP']], new_point[key_indices_1['FN']],
                                   new_point[key_indices_1['FP']], new_point[key_indices_1['TN']],pi0)
        points = insert_point(new_point, key_indices_1, points,
...
def create_prg_curve(labels, pos_scores, pi0, neg_scores=[]):
    ...
    points['recall_gain'] = recall_gain(points['TP'], points['FN'], points['FP'], points['TN'],pi0)
